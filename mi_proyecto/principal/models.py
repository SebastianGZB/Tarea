...
class Empleado(models.Model):
    id_empleado = models.AutoField(primary_key=True)  # NUMBER(10) → clave primaria autoincremental
    ...
    emp_foto = models.BinaryField(null=True, blank=True)   # BLOB → BinaryField en Django
    emp_notas = models.TextField(null=True, blank=True)    # NCLOB → TextField en Django
...
class Orden(models.Model):
    id_orden = models.AutoField(primary_key=True)  # NUMBER(10) → clave primaria autoincremental

    # FK hacia Cliente (ID_CLIENTE → NCHAR(5))
    ...
    # FK hacia Empleado (ID_EMPLEADO → NUMBER(10))
...
class DetalleOrden(models.Model):
    id_detalle = models.AutoField(primary_key=True)  # PK para detalle
